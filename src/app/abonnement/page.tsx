...
export default async function SubscriptionPage() {
  ...
  const canUpgrade = Boolean(
    isActive && proPriceId && currentPriceId && currentPriceId !== proPriceId,
  );
    ...
    <AppShell active="/abonnement" eyebrow="Facturation" title="Abonnement">
      <section className="subscription-layout">
        <article className="workspace-panel subscription-card">
          ...
          {!isActive ? (
            <StripeCheckoutButton action="checkout" label="Passer à Pro" plan="pro" />
          ) : null}
          {isActive ? (
          ...
          {canDowngrade ? (
            <StripeCheckoutButton
              action="downgrade"
              label="Revenir à l'offre Starter"
